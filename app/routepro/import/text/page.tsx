  ...
  function onAnalyze() {
    ...
    try {
      ...
      const normalized = parsed.map((s, i) => ({
        ...
        delivery_window: s.delivery_window ?? null,
        stop_kind: s.stop_kind ?? "delivery",
      }));
      setStops(normalized);
      setAnalyzed(true);
      setShowHelp(false);
    ...
    try {
      ...
      const stopRows = stops
        ...
        .map((s, idx) => ({
          ...
          stop_type: s.stop_kind === "pickup" ? "pickup" : "delivery",
          ...
          <Card className="rounded-2xl">
            <CardContent className="p-3">
              <div className="flex items-center justify-between gap-2">
                <div>
                  <div className="text-sm font-semibold">Import da testo / OCR</div>
                ...
                <Button variant="outline" onClick={() => setShowHelp((v) => !v)}>
                  {showHelp ? "Nascondi" : "Mostra"} guida
                </Button>
              </div>

              {showHelp && (
                <div className="mt-2 rounded-xl border bg-white px-3 py-2 text-xs text-neutral-600">
                  Screenshot Flex → estrai testo → incolla qui → <b>Analizza</b> → correzioni rapide → <b>Crea rotta</b>.
                </div>
              )}
        ...
        {analyzed && (
          <>
            ...
            <ImportReview
              stops={stops}
              onChange={setStops}
              onBackToText={() => setAnalyzed(false)}
            />
          </>
        )}
      </div>

      {/* Mini sticky CTA (molto più basso) */}
      {analyzed && (
            ...
            <CardContent className="p-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <Button onClick={onCreateRoute} disabled={!canCreate}>
                  {saving ? "Creazione..." : `Crea rotta (${stopCount})`}
                </Button>
              </div>
              ...
              <div className="mt-2 flex gap-2">
                ...
                <Button variant="outline" className="flex-1" onClick={() => router.push("/routepro")}>
                  RoutePro
                </Button>
              </div>
            </CardContent>
